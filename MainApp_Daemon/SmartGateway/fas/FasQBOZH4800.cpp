...
FasQBOZH4800::FasQBOZH4800(void)
{
...
bool FasQBOZH4800::init(int nType, const char *sComDevName, int nBitrate, int nParity, int nDatabits, int nStopbits, int nFlowCtrl)
{
...
static unsigned char strStartSymbol[] = {0xC4, 0xEA};	//开始符
	...
	for(int i = 0; i < nLen; i++){
		if(i+1 < nLen){
			if(buffer[i] == strMachineNoSymbol[0] && buffer[i+1] == strMachineNoSymbol[1]){
				nHost = (buffer[i-2] - 0x30)*10 + buffer[i-1] - 0x30;
			}
...
int FasQBOZH4800::handleMsg(PartRunStatus &stPartStatus, const unsigned char * buffer, int nLen)
{
...
int FasQBOZH4800::recvData(int nComType, vector<PartRunStatus> &vtPartStatus)
{
...
int FasQBOZH4800::sendData(int nComType)
{
