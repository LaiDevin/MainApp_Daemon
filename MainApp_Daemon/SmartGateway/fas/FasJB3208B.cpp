...
bool FasJB3208B::init(int nType, const char *sComDevName, int nBitrate, int nParity, int nDatabits, int nStopbits, int nFlowCtrl)
{
...
int FasJB3208B::handleMsg(PartRunStatus &stPartStatus, const unsigned char* buffer, int nLen)
{
	...
	if(bFireAlarm) {
		printf("\n----------------------\n");
		printf("火警\n");
		if(FIRE_ALARM_TYPE_NONE != nFireAlarm){
			printf("%s\n", nFireAlarm == FIRE_ALARM_TYPE_SMOKE?"烟感":"手报");
		}
		printf("点号: %03d\n", nPointNo);
		printf("分区: %04d\n", nPartitionNo);
		printf("机号: %02d\n", nMachineNo);
...
int FasJB3208B::recvData(int nComType, vector<PartRunStatus> &vtPartStatus)
{
...
int FasJB3208B::sendData(int nComType/*zdst define data*/)
{
	return 0;
}
