...
static unsigned char strResetSymbolPart1[] = {0x10,0x22,0x64,0x0C,0xC0,0x44,0x54,0x54,0x54,
                                0x7F,0x54,0x54,0xD6,0x44,0x40,0x00,0x00,0xFE,0x22,
								0x5A,0x86,0x40,0x20,0x50,0x4C,0xC3,0x4C,0x50,0x20,
								0x40};

static unsigned char strResetSymbolPart2[] = {0x04,0x04,0xFE,0x01,0x00,0x00,0xFF,0x15,0x15,
                                0x15,0x55,0x95,0x7F,0x01,0x00,0x00,0x00,0xFF,0x04,
								0x08,0x07,0x22,0x12,0x4A,0x82,0x7F,0x02,0x0A,0x12};


static unsigned char strClearSymbolPart1[] = {0x20,0x10,0x0C,0x03,0xFA,0xAA,0xAA,0xAA,0xAA,0xAA,
						        0xAA,0xFA,0x03,0x02,0x00,0x00,0x80,0x40,0x20,0xF8,
						        0x07,0x10,0xD0,0x10,0x11,0x16,0x10,0x10,0xD8,0x10,
						        0x00,0x00};

static unsigned char strClearSymbolPart2[] = {0x00,0x20,0x90,0x88,0x46,0x47,0x2A,0x32,0x12,0x2A,
...
static bool findReset(const unsigned char * buffer, int nLen)
{
	...
	if(((find(buffer, nLen, strClearSymbolPart1, ARRAY_SIZE(strClearSymbolPart1))) >= 0) &&
		(find(buffer, nLen, strClearSymbolPart2, ARRAY_SIZE(strClearSymbolPart2)) >= 0)) {
		return true;
	}
...
int FasHaiwanGST5000::handleMsg(PartRunStatus &stPartStatus, const unsigned char *buffer, int nLen)
{
...
FasHaiwanGST5000::FasHaiwanGST5000()
{
...
bool FasHaiwanGST5000::unInit(void)
{
    m_obj232Com.Close();
    m_obj485Com.Close();
	return false;
...
bool FasHaiwanGST5000::init(int nType, const char *sComDevName, int nBitrate, int nParity, int nDatabits, int nStopbits, int nFlowCtrl)
{
...
int FasHaiwanGST5000::sendData(int nComType)
{
...
int FasHaiwanGST5000::recvData(int nComType, vector<PartRunStatus> &vtPartStatus)
{
    Com *pobjCom = getComObj(nComType);
    if(NULL == pobjCom) {
        return -1;
