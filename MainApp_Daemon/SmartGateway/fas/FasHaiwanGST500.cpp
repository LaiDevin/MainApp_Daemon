...
int FasHaiwanGST500::handleMsg(PartRunStatus &stPartStatus, const unsigned char *buffer, int nLen)
{
...
FasHaiwanGST500::FasHaiwanGST500()
{
...
bool FasHaiwanGST500::unInit(void)
{
    m_obj232Com.Close();
    m_obj485Com.Close();
	return false;
...
bool FasHaiwanGST500::init(int nType, const char *sComDevName, int nBitrate, int nParity, int nDatabits, int nStopbits, int nFlowCtrl)
{
...
int FasHaiwanGST500::sendData(int nComType)
{
...
int FasHaiwanGST500::recvData(int nComType, vector<PartRunStatus> &vtPartStatus)
{
    Com *pobjCom = getComObj(nComType);
    if(NULL == pobjCom) {
        return -1;
