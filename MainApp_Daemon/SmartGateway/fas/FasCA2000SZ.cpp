...
bool FasCA2000SZ::init(int nType, const char *sComDevName, int nBitrate, int nParity, int nDatabits, int nStopbits, int nFlowCtrl)
{
...
static unsigned char strStartSymbol[] = {0x1B, 0x40};	//开始符
static unsigned char strEndSymbol[] = {0x20, 0x0D};	//结束符

static unsigned char strMachineNoSymbol[] = {0xC7, 0xF8};	//区
static unsigned char strLoopNoSymbol[] = {0x2D};				//“-”
static unsigned char strPointNoSymbol[] = {0xBA, 0xC5};		//号
	...
	for(int i = 0; i < nLen; i++){
		if(i+1 < nLen){
			if(buffer[i] == strMachineNoSymbol[0] && buffer[i+1] == strMachineNoSymbol[1]){
				nHost = (buffer[i-4] - 0x30)*100 + (buffer[i-3] - 0x30)*100 + (buffer[i-2] - 0x30)*10 + buffer[i-1] - 0x30;
			}
			else if(buffer[i] == strLoopNoSymbol[0]){
				nLoop = buffer[i-1] - 0x30;
			}
			else if(buffer[i] == strPointNoSymbol[0] && buffer[i+1] == strPointNoSymbol[1]){
				nPoint = (buffer[i-3] - 0x30)*100 + (buffer[i-2] - 0x30)*10 + buffer[i-1] - 0x30;
			}
		}
...
int FasCA2000SZ::handleMsg(PartRunStatus &stPartStatus, const unsigned char * buffer, int nLen)
{
...
int FasCA2000SZ::recvData(int nComType, vector<PartRunStatus> &vtPartStatus)
{
...
int FasCA2000SZ::sendData(int nComType)
{
